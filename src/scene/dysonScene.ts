...
export function initDysonScene(container: HTMLElement, opts: DysonSceneOptions = {}): DysonSceneApi {
  ...
  // ---------- Galáxia distante (fundo) ----------
  // Textura procedural: núcleo brilhante + braços espirais com estrelas
  // azuis/brancas e regiões HII rosadas. Plano inclinado bem longe, aditivo.
  ...
  const addGalaxy = (tex: THREE.Texture, sizeW: number, pos: [number, number, number], tiltX: number, tiltZ: number, opacity: number) => {
    ...
    gx.lookAt(0, 0, 0); // encara a origem…
    gx.rotateX(tiltX); // …e inclina p/ ler como disco
    gx.rotateZ(tiltZ);
    scene.add(gx);
  };
